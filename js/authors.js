...
class AuthorsManager {
    constructor() {
        ...
        window.authorsManager = this; // Make globally accessible
        this.init();
    }
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    ...
                    <div class="btn-toolbar mb-2 mb-md-0">
                        <div class="btn-group me-2">
                            <button type="button" class="btn btn-sm btn-primary" id="addAuthorBtn">
                                <i class="fas fa-plus"></i> Add Author
                            </button>
                            <button type="button" class="btn btn-sm btn-outline-secondary" id="exportAuthorsBtn">
            ...
            <tr>
                ...
                <td>
                        <div class="btn-group btn-group-sm">
                            <button class="btn btn-outline-primary action-btn" data-action="view" data-id="${author.id}" title="View Details">
                                <i class="fas fa-eye pointer-events-none"></i>
                            </button>
                            <button class="btn btn-outline-secondary action-btn" data-action="edit" data-id="${author.id}" title="Edit">
                                <i class="fas fa-edit pointer-events-none"></i>
                            </button>
                            <button class="btn btn-outline-danger action-btn" data-action="delete" data-id="${author.id}" title="Delete">
    ...
    attachTableEventListeners() {
        ...
        if (addAuthorBtn) {
            addAuthorBtn.addEventListener('click', () => this.showAddAuthorModal());
        }

        // Export Button
        const exportBtn = document.getElementById('exportAuthorsBtn');
        if (exportBtn) {
            exportBtn.addEventListener('click', () => this.exportAuthors());
        }
    ...
    exportAuthorPDF(authorId) {
        ...
        addDetail("Name", author.name);
        addDetail("Nationality", author.nationality);
        addDetail("Lifespan", lifespan);
        addDetail("Books Count", (author.booksCount || 0).toString());

        y += 5;
        doc.setFont("helvetica", "bold");
        doc.text("Biography:", 20, y);
        y += 7;
        doc.setFont("helvetica", "normal");

        const splitBio = doc.splitTextToSize(author.biography || 'No biography available.', 170);
        ...
        doc.save(`${author.name.replace(/[^a-z0-9]/gi, '_')}_details.pdf`);
        authManager.showNotification(`Author details PDF exported for "${author.name}"`, 'success');
    }
}
