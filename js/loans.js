...
class LoansManager {
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    ...
                    <div class="btn-toolbar mb-2 mb-md-0">
                        <div class="btn-group me-2">
                            <button type="button" class="btn btn-sm btn-primary" onclick="loansManager.showAddLoanModal()">
                                <i class="fas fa-plus"></i> New Loan
                            </button>
                            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="loansManager.exportLoans()">
            ...
            <tr class="${isOverdue ? 'table-danger' : ''}">
                ...
                <td>
                    <div class="btn-group btn-group-sm">
                        <button class="btn btn-outline-primary" onclick="loansManager.viewLoanDetails(${loan.id})" title="View Details">
                            <i class="fas fa-eye"></i>
                        </button>
                        ${loan.status === 'active' ? `
                            <button class="btn btn-outline-success" onclick="loansManager.returnBook(${loan.id})" title="Return Book">
                                <i class="fas fa-undo"></i>
                            </button>
                            <button class="btn btn-outline-warning" onclick="loansManager.renewLoan(${loan.id})" title="Renew Loan">
                                <i class="fas fa-sync"></i>
                            </button>
                        ` : ''}
                        <button class="btn btn-outline-danger" onclick="loansManager.deleteLoan(${loan.id})" title="Delete">
    ...
    exportLoanPDF(loanId) {
        ...
        let pdfContent = `Loan Details\n\n`;
        pdfContent += `Book Title: ${loan.bookTitle}\n`;
        pdfContent += `User Name: ${loan.userName}\n`;
        pdfContent += `Loan Date: ${loan.loanDate}\n`;
        pdfContent += `Due Date: ${loan.dueDate}\n`;
        pdfContent += `Return Date: ${loan.returnDate || 'Not returned yet'}\n`;
        pdfContent += `Status: ${loan.status}\n`;
        pdfContent += `Fine: $${loan.fine.toFixed(2)}\n`;
        pdfContent += `Renewal Count: ${loan.renewalCount}\n`;
        pdfContent += `Notes: ${loan.notes || 'None'}\n`;

        const blob = new Blob([pdfContent], { type: 'text/plain' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `loan_${loan.id}_details.txt`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        window.URL.revokeObjectURL(url);

        authManager.showNotification(`Loan details exported for "${loan.bookTitle}"`, 'success');
    }
}
